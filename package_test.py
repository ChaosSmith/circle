...
def test_get_packages(player):
        print("Can {player_name} get the packages?".format(player_name = player.name))
        params = {'api_key': player.api_key}
        response = requests.get(host + "packages", params=params)
        print(response.text)
...
# Individual Agents
test_get_packages(alpha)
test_get_packages(bravo)
test_get_packages(charlie)
